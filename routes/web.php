...
/*
|--------------------------------------------------------------------------
| DASHBOARD (ROLE REDIRECT LATER IN CONTROLLER)
|--------------------------------------------------------------------------
*/
Route::get('/dashboard', function () {
    ...
    // Attendance last 7 days by status
    $week = collect();
    for ($i = 6; $i >= 0; $i--) {
...
// Student pending (no student.linked middleware here — it IS the fallback)
Route::middleware(['auth'])->group(function () {
    Route::get('/pending', function () {
        // If they are now linked, send them to the portal
        $linked = \App\Models\Student::where('email', auth()->user()->email)->exists();
        if ($linked) {
...
// Student portal — guarded: must be linked
...
// Admin: link user accounts to student records
...
// Admin/Teacher: browse all student results
Route::middleware(['auth', 'permission:view results|manage exams'])->group(function () {
...
Route::middleware('auth')->group(function () {
    ...
    */
    ...
    // Students — create before {student} to avoid route collision
    ...
    // Timetables — view for teachers+students, manage for admin
    ...
    // Teachers — create before {teacher}
    ...
    // Classes — create before {school_class}
    ...
    // Subjects — create before {subject}
    ...
    // Exams — create/store must be registered BEFORE {exam} to avoid route collision
    Route::middleware(['permission:manage exams'])->group(function () {
    ...
    // Reports — view results or manage exams
    Route::middleware(['permission:view results|manage exams'])->group(function () {
